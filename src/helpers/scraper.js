...
class Scraper {
    ...
    static searchBySongs(songTitle, lyrics, start, length){
        return new Promise((resolve, reject) => {
            const searchUrl = 'https://www.metal-archives.com/search/ajax-advanced/searching/songs/?songTitle=' + songTitle;
            console.log(searchUrl);
            axios.get(searchUrl)
                .then(({ data }) => {
                    const totalResult = data.iTotalRecords;
    ...
    // static searchByBands(bandName) {
    //     return new Promise((resolve, reject) => {
    //         const searchUrl = `${SEARCH_SONGS_URL}?bandName=${bandName}`;
